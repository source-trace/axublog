...
$cjhtml=file_get_contents('http://www.baidu.com/s?rn=100&wd='.$cjtag);
...
$dtitle=$durl.'网站快照：'.$dtitle;
#die($dtitle);



$pattern = '<meta name="keywords" content="">';
preg_match('/(<meta name="keywords" content=")(.+)(")/',$cjhtml,$matchs);
$webtags=trim($matchs[2]);
		if(str_replace(',','',$webtags)==$webtags){$webtags=trim($cjtag);}
		else{
		$webtags=$webtags.',';
		$webtags=str_replace(',,',',',$webtags);
		$webtags=str_replace($cjtag.',','',$webtags);
		$webtags=trim($cjtag.','.$webtags);}
...
$dcontent='<p>网址：'.$cjurl.'</p><p>网站title：'.$dtitle.'</p><p>网站简介：</p>'.$cjhtml;
...
$_SESSION['date']=$date;
$_SESSION['author']=$durl;
$_SESSION['htmlname']=$htmlname;
$_SESSION['tags']=$webtags;
